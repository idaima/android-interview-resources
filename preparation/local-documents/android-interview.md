...
整理自原仓库的 Android-1 与 Android-2 文档。
...
![framework](assets/android-interview/framework.png)
...
![pic1](assets/android-interview/activity1.png)

![pic2](assets/android-interview/activity2.png)
...
![fragment1](assets/android-interview/fragment1.png)
...
![fragment2](assets/android-interview/fragment2.png)
...
![service1](assets/android-interview/service1.png)
...
![service2](assets/android-interview/service2.png)

Service生命周期图三：
![service3](assets/android-interview/service3.PNG)
...
![sqlite1](assets/android-interview/sqlite1.png)
...
![sqlite2](assets/android-interview/sqlite2.png)
...
Cursor 是查询到的数据条目的集合，相当于JDBC中的ResultSet。Cursor使用如下方法来移动查询结果的记录指针：
 ...
 ![sqlite3](assets/android-interview/sqlite3.png)
...
### 1.从进程角度看IPC机制：![binder5](assets/android-interview/binder5.png)
...
![binder6](assets/android-interview/binder6.png)
...
![binder3](assets/android-interview/binder3.PNG)

Binder工作机制

![binder4](assets/android-interview/binder4.png)
...
![img](assets/android-interview/binder1.png)

**Binder各组件之间的关系：**

![binder2](assets/android-interview/binder2.png)
...
![view1](assets/android-interview/view1.png)
...
![view2](assets/android-interview/view2.jpg)
...
![view3](assets/android-interview/view3.png)

只有ViewGroup才能对事件进行拦截：

![view4](assets/android-interview/view4.png)



如果Activity的所有子View都不处理事件，则最后会调用Activity的onTouchEvent():

![view5](assets/android-interview/view5.png)

如果在最底层View处理：

![view6](assets/android-interview/view6.png)

如果在中途View处理了该事件，则不会继续向下进行事件分发：

![view7](assets/android-interview/view7.png)
...
![anim1](assets/android-interview/anim1.png)
...
![anim3](assets/android-interview/anim3.png)
...
![anim4](assets/android-interview/anim4.png)
...
![anim5](assets/android-interview/anim5.png)
...
![activity3](assets/android-interview/activity3.jpg)
...
![handler1](assets/android-interview/handler1.png)

ThreadLocal的`set` 方法：实际上就是往ThreadLocalMap对象(map)维护的对象数组table中插入数据。

![handler2](assets/android-interview/handler2.png)

ThreadLocal的`get` 方法，调用了ThreadLocalMap的getEntry()方法：

![handler3](assets/android-interview/handler3.png)

ThreadLocalMap的`getEntry()` 方法:

![handler4](assets/android-interview/handler4.png)
...
![looper](assets/android-interview/looper.png)

Looper的构造方法，在构造方法中，创建了一个**MessageQueue** 实例：

![looper1](assets/android-interview/looper1.png)

当需要为一个线程创建Looper对象时，需要调用Looper的`prepare()` 方法（该方法在一个线程中只能调用一次，否则会抛出异常）：

![looper2](assets/android-interview/looper2.png)

在`loop()` 的消息循环中，实际上是调用了MessageQueue的**next()** 方法。

![looper3](assets/android-interview/looper3.png)

![looper4](assets/android-interview/looper4.png)
...
![handler5](assets/android-interview/handler5.png)

![handler6](assets/android-interview/handler6.png)


![handler7](assets/android-interview/handler7.png)

最后调用了MessageQueue的`enqueueMessage()` 函数：

![handler8](assets/android-interview/handler8.png)

![handler9](assets/android-interview/handler9.png)
...
#### JVM体系结构：
![jvm4](assets/android-interview/jvm4.png)
...
![dalvik1](assets/android-interview/dalvik1.png)

#### Android应用的编译以及运行流程
![dalvik2](assets/android-interview/dalvik2.png)
...
* `forkSystemServer()`  ：Fork一个系统服务进程。
  ...
  ![dalvik3](assets/android-interview/dalvik3.png)


#### Dalvik内存管理
Dalvik的内存管理需要依赖Linux的内存管理机制，垃圾回收是采用了标记-清除法，使用标志位来标识内存中的对象是否正在使用。Dalvik是采用 **分离式的标志位方案**：
![dalvik4](assets/android-interview/dalvik4.png)
...
#### 打包安装运行简化流程
![dalvik5](assets/android-interview/dalvik5.png)
...
![boot2](assets/android-interview/boot2.png)
...
![boot1](assets/android-interview/boot1.png)
...
![sophix1](assets/android-interview/sophix1.png)
...
当sdk>=23时，只要把补丁so库的完整路径作为参数构造一个`Element` 对象，然后再插入到nativeLibraryElements数组最前面就可以达到修复的目的。
 ...
 ![Sophix2](assets/android-interview/sophix2.png)
