...
**答：** Composable 在读取 `State` 时会建立观察关系；该 State 写入新值后，读取它的重组作用域被标记失效，Recomposer 在帧调度中再次执行相关 Composable。重组会比较参数、复用 SlotTable 中同位置的 remember 值，并只把必要差异应用到 UI；未读取该 state 或可安全跳过的子树不会重组。state 应是唯一真源，直接改普通变量不会通知 Runtime。
...
**答：** 状态提升把 value 和 `onValueChange` 暴露给调用方，组件变成无状态、由输入决定输出的受控 UI；不同页面、ViewModel、Preview 或测试可复用同一组件并注入不同 state。业务规则和状态归属更清晰，单测可直接断言事件和渲染结果。通常将状态提升到所有读取和修改它的调用方的最低共同祖先，而不是无条件提升到全局。

**问：** 哪些情形下会避免状态提升，而将状态保留在 Composable 内部？

**答：** 仅影响组件内部表现、调用方既不需要读取也不需要控制的短暂状态可内部保存，如 ripple、展开动画进度、一个输入框的临时焦点或滚动动画状态。把这类实现细节上提会扩大 API、增加重组范围并让父级承担无意义逻辑。若状态需跨组件共享、由业务规则决定、跨配置/进程恢复或需要测试控制，则应提升到合适的状态持有者或 ViewModel。
...
**答：** `remember` 只在当前 Composition 存活期间保留值；需要跨 Activity 重建、配置变更和系统进程重建恢复的小型 UI 状态时用 `rememberSaveable`，如当前 tab、输入内容、滚动位置或选中 ID。它通过 SavedStateRegistry/Bundle 保存，受可保存类型和 Binder 大小限制，写入过大对象会影响恢复甚至失败。网络数据、图片、大列表和业务真源应放 ViewModel、数据库或仓库，而非 `rememberSaveable`。

**问：** 如何用 `rememberSaveable` 保存默认不支持的自定义非基本类型状态？

**答：** 为类型提供 `Saver`：用 `listSaver`/`mapSaver` 将其转换为 Bundle 可保存的基本值、String、Parcelable 等，恢复时重建对象；或实现 `Parcelable`/`Serializable`（Android 上优先 Parcelable）后配合适当 Saver。Saver 必须只保存恢复 UI 所需的最小数据，并处理版本兼容与缺失值，例如只保存选中项 ID 而不保存完整对象图。不可保存资源、协程、Context、Bitmap 等不能强行保存。
...
**答：** 函数体会随重组反复执行，直接 `launch` 会重复请求、重复收集或泄漏，且无法随 Composition 自动取消。与 Composition 绑定的挂起工作用 `LaunchedEffect(key)`，可清理的非挂起订阅用 `DisposableEffect`，用户点击等事件触发的任务用 `rememberCoroutineScope()`，业务长期任务放 ViewModel 的 `viewModelScope`。key 应稳定且代表任务输入，避免用频繁变化对象导致不断重启。
...
**答：** `LaunchedEffect` 在进入 Composition 时创建一个绑定 Composition 的协程来运行挂起代码；离开 Composition 时自动取消。key 未变时重组不会重启该协程；任一 key 改变时旧协程先取消，再以新 key 启动新协程，因此适合加载某个 ID、收集某个输入或延迟操作。使用常量 key 等同于该调用点生命周期，只能在确实需要一次性任务时使用；不要把不稳定对象作为 key。

**问：** 何时应使用 `DisposableEffect` 而不是 `LaunchedEffect`？

**答：** 当 API 的模式是注册/注销而非挂起协程时使用 `DisposableEffect`，例如添加 `LifecycleObserver`、位置监听器、BroadcastReceiver 或第三方 callback；在 effect 块注册，并在 `onDispose` 中必定取消注册/释放资源。`LaunchedEffect` 用于挂起函数、Flow collect、动画等协程工作，取消由协程结构化并发完成。两者 key 改变都会先清理/取消再重建，不能把空 `onDispose` 的逻辑硬塞进 DisposableEffect。

**问：** 请说明 `SideEffect` 的使用场景，以及它与 `LaunchedEffect` 的区别。

**答：** `SideEffect` 在一次成功重组提交后于主线程执行，用于把最新 Compose state 同步给非 Compose 对象，例如更新 analytics 属性、View 的简单字段；它不提供协程、取消或 key，每次成功重组都会执行。`LaunchedEffect` 启动可挂起的协程，只在首次进入或 key 改变时运行，并在离开/变 key 时取消。耗时、异步或一次性任务不应放进 SideEffect。
...
**答：** 用 `val currentCallback by rememberUpdatedState(callback)` 保存每次重组后的最新 lambda，而让 `LaunchedEffect` 的 key 只包含真正应重启延迟的值；延迟结束后调用 `currentCallback()`。若把 callback 放入 key，父级每次创建新 lambda 都会取消并重启延迟；若完全捕获旧 callback，则可能执行过期逻辑。`rememberUpdatedState` 适合这种长生命周期 effect 读取最新值的场景，不替代真正需要重启的 key。
...
**答：** 使用 `produceState(initialValue, key...) { value = repository.load() }`：它为 producer 创建随 Composition 生命周期取消、随 key 改变重启的协程，并把 `value` 暴露为 `State` 供 UI 读取。若数据本来是 Flow，可用 `collectAsStateWithLifecycle()`（Android）或 `collectAsState()` 转为 State；业务加载和缓存通常更适合先由 ViewModel 暴露 StateFlow。不要为每次重组创建新的 producer 或在 producer 中持有已失效 Context。
...
**答：** `snapshotFlow` 用于把 Compose Snapshot 中的状态转为 Flow，例如 `LazyListState.firstVisibleItemIndex`、动画值或某个 Compose 控件状态，以便在 `LaunchedEffect` 中做滚动分析、分页阈值或日志；业务数据应仍由 ViewModel Flow 提供，不要反向依赖 UI snapshot。它会在读取结果变化时发射，常结合 `map`、`distinctUntilChanged()`、`filter`、`debounce` 或 `collectLatest` 降低高频事件；在 effect 中收集，确保离开 Composition 自动取消。
...
**答：** 当输入变化频繁而派生结果变化较少，并且该结果被 Compose 读取时用 `derivedStateOf`，如滚动位置映射为“是否显示返回顶部”布尔值；它可减少依赖方失效。简单字符串拼接、少量字段相加或输入每次变化都会导致结果变化时，直接计算更清晰，额外 derived state 没有收益。它不是缓存昂贵计算的万能工具，也不应用来替代在 ViewModel/Flow 中处理业务派生数据。
...
**答：** Composable 会进入 Composition、在需要时参与重组、其节点经历 measure/layout/draw，并最终离开 Composition；这不是类似 Activity 的固定回调序列，同一调用可被跳过或多次执行。首次执行建立状态读取和 remember，状态变化使读取点失效，Recomposer 在下一帧重组最小范围、应用差异并按需重新布局/绘制；离开时 `LaunchedEffect` 取消、`DisposableEffect` 清理。应以 Effect API 管理生命周期资源而非假设函数只运行一次。
...
**答：** 让每个标签拥有稳定 key 和独立状态持有者：例如在父级以 `rememberSaveable(tabId)` 保存筛选/输入，用 `rememberLazyListState()` 并配合 `LazyListState.Saver` 保存滚动位置；切换时保留各标签 Composition，或将状态提升到按 tabId 映射的 ViewModel/SavedStateHandle。不要只用一个共享 state 覆盖所有标签，也不要依赖被移出 Composition 的普通 `remember`。跨进程恢复仅保存小状态，列表数据仍由仓库重新提供。
...
**答：** `Snapshot.takeSnapshot()` 适合非 UI、只读且需要一致视图的场景，例如后台导出当前多个 Compose state 的一致配置、调试工具检查一组状态，或测试中在不触发 Composition 的情况下读取快照。它捕获读取时刻的一致数据，随后需要 `dispose()`，并须遵守 Snapshot 的线程与可变状态规则。普通 UI 渲染应直接在 Composable 中读取 state，让 Runtime 自动订阅和重组；不要用手工 Snapshot 绕过响应式模型。
...
**答：** `mutableStateOf(list)` 只观察 list 引用本身；对其中普通 `MutableList` 调用 `add/remove` 不会给外层 State 赋新值，因此不会失效。可改用 `mutableStateListOf()`/`SnapshotStateList`，或保持不可变集合并在更新时 `state.value = state.value + item`、`filterNot` 创建新引用；复杂业务列表更适合 ViewModel 暴露不可变 `StateFlow`。不要通过强制重新赋同一引用或手动 invalidate 修补。

**问：** 在 `LazyColumn` 中动态添加、删除列表项时，如何高效追踪 UI 更新？

**答：** 使用可观察的不可变列表或 `SnapshotStateList` 作为数据源，并在 `items(items, key = { it.id }, contentType = { ... })` 提供稳定业务 key。添加/删除时仅更新数据源，LazyColumn 会依据 key 保留未变化条目的 remember、动画和滚动锚点；可配合 `animateItem()` 实现插入/删除过渡。避免用 index 当 key、原地修改不可观察集合或每次 `notify` 式全量重建；分页和筛选应在 ViewModel 维护一致的列表状态。
...
**答：** Android UI 中优先使用 `collectAsStateWithLifecycle()`（lifecycle-runtime-compose），它默认在 `STARTED` 时通过 `repeatOnLifecycle` 收集，停止时取消收集并在恢复时重订阅；也可在 ViewModel/Fragment 使用 `repeatOnLifecycle` 后暴露 state。对必须持续的热流，明确其资源成本和 `SharingStarted` 策略。不要把生命周期判断散落在每个 Composable，也不要误以为 `collectAsState` 会自动停止上游工作。
...
**答：** `CompositionLocal` 是沿 Composition 树隐式提供/读取的值，如 Material 的颜色、排版、布局方向和密度。适合真正的横切环境依赖、主题 token、当前 content color 或少量框架级服务，能避免中间层只为转发参数而污染 API。业务数据、屏幕 state、事件回调和 ViewModel 应显式传参或注入，因为隐式依赖难追踪、难测试且可能让重组范围不透明。优先使用现有 Local，谨慎自定义。

**问：** `CompositionLocalProvider` 如何工作？若访问未提供值的 `CompositionLocal`，会发生什么？

**答：** `CompositionLocalProvider(LocalFoo provides value) { ... }` 在其子树建立值的作用域，嵌套 Provider 可覆盖外层值，子 Composable 通过 `LocalFoo.current` 读取当前最近提供者。`compositionLocalOf` 必须给出默认值；若默认值设计为抛异常，则未提供时访问会立即暴露配置错误。`staticCompositionLocalOf` 适合很少变化的值，`compositionLocalOf` 会更细粒度追踪读取；无论哪种，都不应用 Provider 频繁传递高频业务 state。
