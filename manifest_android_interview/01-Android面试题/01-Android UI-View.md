...
**答：** 构造函数只解析属性并完成轻量字段初始化；依赖最终尺寸的对象放在 `onSizeChanged()`，不要在频繁调用的 `onDraw()` 分配或加载。需要在 View 真正可见时运行的图片请求、动画和监听器可在 `onAttachedToWindow()` 启动，尺寸变化时重新计算；若资源只依赖宿主的 `LifecycleOwner`，也应遵循其生命周期。必须在 `onDetachedFromWindow()` 取消图片/协程请求、停止动画、移除回调和监听器、释放临时资源；不要让静态对象、长生命周期任务或 Runnable 持有 View/Activity。若使用 Glide、Coil 等库，应把请求绑定到 View 或宿主生命周期，让其自动取消。

**问：** 应用拥有复杂 UI，其中动态创建的 View 存在性能问题。如何优化 `onMeasure()` 和 `onLayout()`，在保持响应性的同时提高渲染效率？

**答：** `onMeasure()` 必须正确处理 `MeasureSpec` 的 `EXACTLY`、`AT_MOST` 和 `UNSPECIFIED`，根据子 View 的真实需求计算尺寸后调用一次 `setMeasuredDimension()`；缓存与尺寸无关或可复用的计算结果，避免在其中创建对象、访问磁盘或反复测量同一子 View。`onLayout()` 只在位置确实改变时布局子 View，避免无条件 `requestLayout()` 形成反复测量；动态增删 View 应批量进行并使用扁平层级、`ConstraintLayout`、`ViewStub` 或 RecyclerView 取代深层嵌套和大量一次性子项。用 Layout Inspector、Systrace/Perfetto、JankStats 检查 measure/layout 次数和耗时；优化目标是减少不必要的遍历，而不是绕过正确测量。
...
**答：** `requestLayout()` 表示 View 的期望尺寸或位置变了，向父级请求下一帧重新执行 measure/layout，适用于修改影响几何结构的属性，例如内容导致高度变化。`invalidate()` 标记 View 的全部或部分区域需要在主线程的下一次绘制阶段重画，但不会重新测量或布局，适合颜色、进度、绘制数据等仅影响外观的变化。`postInvalidate()`/`postInvalidateOnAnimation()` 可从非 UI 线程安全地请求重绘（后者对齐下一帧）；它们不是把任意 View 修改变为线程安全，状态写入仍应切回主线程或做好同步。不要在 `onDraw()` 中无条件调用这些方法，否则会造成持续重绘。

**问：** View 生命周期与 Activity 生命周期有何不同？为什么理解二者对高效 UI 渲染都很重要？

**答：** Activity 生命周期描述页面组件从创建、可见、前台到停止/销毁的状态；View 生命周期描述某个视图何时被构造、附着到 Window、测量、布局、绘制、脱离 Window。View 可在同一 Activity 内多次创建和销毁，例如 Fragment 位于返回栈时其 View 已销毁，或 RecyclerView 条目被回收；Activity 存活并不意味着某个 View 仍可用。理解二者才能把尺寸计算放到合适回调、仅在附着时启动动画/监听、在 View 销毁时取消 UI 工作，并避免持有过期 binding 或在不可见界面做无效绘制。
...
**答：** `ViewStub` 初始是几乎零成本的占位 View，不会 inflate 其 `layout` 指向的真实层级；调用 `inflate()` 或把它设为可见后，系统解析目标布局，并用生成的根 View 在父容器中替换该 Stub。因而首次进入页面时可少创建、测量和绘制低频区域，降低初始内存和首帧开销；代价是首次展示该区域会有一次 inflate 延迟，适合错误页、空状态、折叠面板等非立即需要的 UI。填充后 Stub 不再存在，应保存 inflate 返回的 View 或使用 `inflatedId`；不要把每次显示/隐藏都交给 ViewStub，已创建区域应复用或按需移除。
...
**答：** 在 `attrs.xml` 定义 `declare-styleable` 与属性类型，View 提供 `Context, AttributeSet?, defStyleAttr, defStyleRes` 形式的构造入口；使用 `context.obtainStyledAttributes(attrs, R.styleable.YourView, defStyleAttr, defStyleRes)` 读取 XML、主题和默认样式的合并结果，并始终在 `finally` 中 `recycle()`。将解析后的值缓存为字段，仅在值变化时 `requestLayout()` 或 `invalidate()`，不要在 `onDraw()` 反复读取资源。通过 `defStyleAttr`/`defStyleRes` 提供稳定默认值、正确处理缺省属性和 `@null`，使用 AndroidX/资源限定符处理版本差异；新增属性保持可选且不改变旧 XML 的默认行为，从而兼容旧布局和主题覆盖。
...
**答：** 继承 `View`（需要管理子项才继承 `ViewGroup`），在构造函数初始化可复用的 `Paint`、`Path` 等对象，在 `onMeasure()` 报告合理尺寸，在 `onSizeChanged()` 根据宽高构建几何缓存，并在 `onDraw(canvas)` 完成渲染。Canvas 常用 `drawPath()`、`drawRoundRect()`、`drawCircle()`、`drawText()`、`drawBitmap()`；用 `save()`/`restore()` 包围 `translate()`、`rotate()`、`scale()`、`clipPath()` 等局部变换，使用 `Paint` 的抗锯齿、Shader、ColorFilter 和 `PorterDuff`/BlendMode 控制效果。交互元素还应实现触摸状态、无障碍语义和状态保存，避免每帧分配对象；优先使用硬件加速友好的 API，必要时用 `RenderEffect`、`Drawable` 或 Compose 评估是否比自绘更合适。
...
**答：** `invalidate()` 在调用线程为主线程时标记 View 的脏区域，系统会在下一帧遍历时调用其 `onDraw()`；例如进度条收到主线程状态更新后改变进度并重绘。`postInvalidate()` 将同一重绘请求投递到 View 所在线程的消息队列，可由工作线程调用；例如音频解码线程更新波形数据后请求波形 View 刷新。两者都只请求绘制，不负责重新布局；动画场景更适合 `postInvalidateOnAnimation()` 或 `ValueAnimator`，以按屏幕帧率合帧。后台线程传入的数据必须在主线程读取前安全发布，不能一边修改集合一边绘制。

**问：** 若要从后台线程更新 UI 元素，如何确保重绘操作安全地在主线程执行？

**答：** Android View 不是线程安全的，后台线程只能计算不可变结果或更新受保护的数据，最终通过 `view.post { ... }`、`Handler(Looper.getMainLooper())`、`Activity.runOnUiThread()`，或协程的 `withContext(Dispatchers.Main)` 更新 View 并调用 `invalidate()`。提交前要确认 View 仍附着、Fragment 的 View 生命周期仍有效，或让任务随 `viewLifecycleOwner` 自动取消，避免页面销毁后更新旧 View。使用 LiveData/StateFlow 时在 UI 层以生命周期感知方式收集，也能确保仅在活跃界面渲染；切勿从后台直接调用 `setText()`、修改 LayoutParams 或执行 View 动画。
...
**答：** ConstraintLayout 允许多数 View 直接约束到同一父容器，用约束求解一次确定位置和尺寸，通常可替代多层 LinearLayout/RelativeLayout 的嵌套，从而减少 measure/layout 遍历和 View 层级深度。它特别适合有相对关系、对齐、比例、链、Barrier、Guideline 或响应不同屏幕尺寸的复杂卡片、表单和工具栏；MotionLayout 也以它为基础。性能收益来自减少不必要嵌套而非“任何布局都更快”：简单的一行/一列 LinearLayout 可读性更好且足够高效，过多约束、嵌套 ConstraintLayout 或滥用复杂 Helper 同样会增加求解成本。

**问：** ConstraintLayout 中 `match_constraint`（`0dp`）如何工作？与 `wrap_content`、`match_parent` 有何不同？在什么场景使用？

**答：** 在 ConstraintLayout 中，某一维设为 `0dp` 表示 `match_constraint`：该维不自行决定大小，而由两端约束、百分比、ratio、最小/最大值或链规则求解；例如左右都约束到父容器时宽度填满两者之间的可用空间。`wrap_content` 按内容的固有尺寸测量，`match_parent` 填满父容器，且在 ConstraintLayout 中通常不如用两端约束加 `0dp` 灵活，甚至会忽略部分约束意图。使用 `0dp` 实现自适应宽高、等分链、比例图片和边界间拉伸；若只约束一侧而没有足够规则，尺寸可能无法按预期求出，应补齐约束或选用 `wrap_content`。
...
**答：** 通过 `holder.addCallback()` 监听 `SurfaceHolder.Callback`：在 `surfaceCreated()` 后再创建相机、播放器或渲染线程并开始向有效 Surface 输出；在 `surfaceChanged()` 更新缓冲尺寸/渲染参数；在 `surfaceDestroyed()` 立即停止渲染、解除相机/解码器输出、释放或退出线程，之后不再访问该 Surface。渲染循环应检查 `holder.surface.isValid`，对 `lockCanvas()`/`unlockCanvasAndPost()` 使用 `try/finally` 保证解锁，并避免在主线程绘制重帧。还要把播放/相机行为与 Activity/Fragment 生命周期、权限和前后台状态协同处理，在 View 脱离或页面销毁时注销 callback、取消任务；不要缓存已销毁的 Surface 或 Context。

**问：** 若要显示可旋转、缩放的实时相机预览，应在 SurfaceView 与 TextureView 中选择哪个？请结合实现考虑说明原因。

**答：** 若预览必须像普通 View 一样进行任意旋转、缩放、裁剪、淡入淡出或与其他 View 做复杂动画，通常选择 `TextureView`：它提供 `SurfaceTexture`，可通过 View 的 transform/Matrix 参与 View 层级合成，便于按相机和显示方向做变换。代价是内容需经过 GPU 合成，通常比 `SurfaceView` 多一层开销；`SurfaceView` 由独立 Surface 合成，适合追求低延迟、稳定高分辨率视频或全屏预览，但传统上不适合任意 View 变换且层叠限制更多。实际项目还应优先评估 CameraX `PreviewView`，它会按设备和实现选择合适模式；无论选择哪种，都要在 Surface 可用时绑定相机、销毁时解除绑定并正确处理显示旋转。
...
**答：** RecyclerView 将条目 View 的引用封装在强制使用的 `ViewHolder` 中，并通过回收池复用离屏条目，滚动时只创建少量必要 View、再绑定新数据，避免 `findViewById()` 和重复 inflate。ListView 也可借助 `convertView` 与手写 ViewHolder 实现复用，但 RecyclerView 将该模式、布局管理、动画和条目装饰解耦为标准 API，支持 `ListAdapter`/`DiffUtil` 做增量更新，减少 `notifyDataSetChanged()` 引起的全列表重绑。性能仍取决于 `onBindViewHolder()` 是否轻量、图片加载是否可取消、item 层级是否扁平；ViewHolder 本身不能解决昂贵绑定或频繁全量刷新。

**问：** 请说明 RecyclerView 中 ViewHolder 从创建到复用的生命周期。

**答：** 当屏幕需要某类条目且缓存/回收池没有可用 Holder 时，RecyclerView 调用 `onCreateViewHolder()` inflate 并创建 Holder；随后调用 `onBindViewHolder()`（或带 payload 的重载）把当前位置的数据完整绑定到它。Holder 可经历 `onViewAttachedToWindow()` 与 `onViewDetachedFromWindow()`，滚出屏幕后会先进入缓存或回收池；真正回收时会回调 `onViewRecycled()`，之后可能被取出并再次 bind 到完全不同的位置。因而 bind 必须覆盖所有可见状态，不能依赖旧位置残留；在 detached/recycled 时取消图片请求、动画、监听或异步回调，`getBindingAdapterPosition()` 仅在需要即时响应点击时读取并检查 `NO_POSITION`。

**问：** 什么是 RecycledViewPool？如何用它优化条目 View 渲染？

**答：** `RecycledViewPool` 是 RecyclerView 按 `viewType` 保存已回收 ViewHolder 的共享池；同一 RecyclerView 默认已有池，多个结构相同的 RecyclerView（例如外层列表中每行都有横向列表）可通过 `setRecycledViewPool(pool)` 共用它，减少反复 inflate 和创建 Holder。可根据稳定的 ViewType 调用 `setMaxRecycledViews(type, max)` 调整容量，并在嵌套列表场景设置合理 `initialPrefetchItemCount`。共享的前提是 ViewType 对应的布局和绑定逻辑完全兼容；不要让不同 Adapter 复用相同数值 ViewType 却有不同布局，也不要把池无限调大，否则会用内存换取很少的收益。先通过滚动 profile 证实创建成本是瓶颈。
...
**答：** `sp` 会随系统字体缩放，用户把字号调大时，固定高度的 TextView/按钮、单行强制截断、紧凑的横向排列和依赖文本宽度的图标可能溢出、重叠或被裁剪；这不是 `sp` 的错误，而是布局未适配无障碍字体。应使用 `wrap_content`、可换行/可滚动容器、约束或流式布局，为重要文字留出扩展空间，并设置合理的最小触控尺寸；必要时用 `maxLines`、省略号和可访问完整内容的方式做有意识降级。需在最大字体/显示大小、不同语言和横竖屏实测，不能改用 `dp` 或强行忽略系统 fontScale 来掩盖问题。
...
**答：** Nine-Patch 是扩展名为 `.9.png` 的特殊 PNG，四周 1 像素标记定义可拉伸区域和内容 padding；Android 绘制时只拉伸标记的中间区域，保留边框、圆角和阴影等不应变形的部分。普通 PNG 被整体缩放，放大后边角和边框可能失真。它适合需要适应不同文本长度的气泡、对话框、带边框的按钮背景等位图样式；简单圆角、描边和颜色状态优先用 shape/ripple Drawable，纯图标优先 VectorDrawable。制作时应使用正确的 1 像素边界标记并避免把该边界显示为内容。
...
**答：** 使用 XML drawable 组合状态和外观：以 `shape` 定义圆角、描边、渐变或填充颜色，用 `selector` 针对 `state_enabled`、`state_selected`、`state_checked` 等状态切换不同 shape/颜色；在 Android 5.0+ 将 `ripple` 作为外层以提供按压波纹，并通过 `color` 或 `ColorStateList` 定义状态色。把它设为 `android:background`，同时保留可见文本、足够对比度和最小触控区域；使用 `android:foreground` 可在不替换背景时叠加 ripple。若使用 MaterialButton，优先利用其 `backgroundTint`、`shapeAppearance` 和状态色，避免覆盖背景后丢失 Material 的状态与内边距行为。
...
**答：** 解码后的 Bitmap 占用的是像素内存而非压缩文件大小：例如 ARGB_8888 每像素约 4 字节，超大图片、多张同时加载或重复副本会造成频繁 GC、卡顿甚至 `OutOfMemoryError`。应先用 `BitmapFactory.Options.inJustDecodeBounds` 获取原尺寸，再按目标显示大小计算 `inSampleSize` 采样解码；选择合适配置（不需要透明且可接受质量时可评估 `RGB_565`），避免在主线程解码、避免不必要的 Bitmap 拷贝，并及时关闭输入流。实际列表和网络图片应使用 Glide、Coil、Picasso 等库，它们提供尺寸感知解码、内存/磁盘缓存、请求取消和复用；图片请求应绑定生命周期，缓存要有上限，不能依赖手动 `recycle()` 作为常规内存管理手段。
...
**答：** 对按压反馈使用 `ViewPropertyAnimator` 或 `StateListAnimator` 动画 `scaleX`、`scaleY`（必要时加 `translationZ`/alpha），例如按下动画到 `0.96f`、松开或取消时回到 `1f`；这些属性通常可由 RenderNode/GPU 高效合成，不会触发重新 measure/layout。可在 `setOnTouchListener` 处理 `ACTION_DOWN`、`UP`、`CANCEL`，但必须返回 `false` 或完整保留 click、无障碍和取消语义；Material 组件还应保留 ripple。避免逐帧修改 `LayoutParams`、频繁 `requestLayout()` 或在动画回调中创建对象；设置较短时长与合适插值器，并在 View detach 时取消动画，防止复用/离开页面后状态错误。

**问：** 何时应使用 MotionLayout 而非传统 View 动画？它有哪些优势？

**答：** 当多个 View 的位置、尺寸、透明度和约束需要在两个或多个界面状态间协调变化，或动画需要由拖拽、滚动、折叠进度交互驱动时，使用 MotionLayout；它把 `ConstraintSet`、Transition、关键帧和触摸响应集中在 MotionScene 中，可按 `progress` 精确控制并自动处理约束插值。传统 `ViewPropertyAnimator`/`ValueAnimator` 更适合单个、独立且短小的属性反馈，例如按钮缩放、淡入淡出，代码更简单、开销更低。MotionLayout 的优势是声明式编排、可预览/调试、支持过渡监听和复杂路径，但场景文件与状态维护也更复杂；不要仅为一个 alpha 动画引入它。
...
**答：** 对一个普通 Activity 而言，应用通常只有一个由 `PhoneWindow` 表示的 Activity Window；它向 WindowManager 添加一个根 `DecorView`，应用的 `setContentView()` 内容、ActionBar/Toolbar 所在的内容区域都属于这个 Window。状态栏、导航栏、输入法、通知栏等由 System UI/IME 等系统进程管理的独立系统 Window/层负责，并不意味着每个 View 或每个布局都有一个 Window；启用 edge-to-edge 后应用内容可以绘制到系统栏区域下方，但 Activity 仍是同一个 Window。Dialog、PopupWindow、Toast（实现因版本而异）等额外 UI 才可能请求额外 Window 或系统层。
...
**答：** 为 WebView 设置 `WebViewClient`，在 `shouldOverrideUrlLoading()` 中解析 `request.url`：对允许在应用内浏览的 `http/https` 域名白名单返回 `false` 让 WebView 正常加载；对不受信任域名、`tel:`、`mailto:`、市场链接或自定义 scheme，按产品策略使用显式/受校验的外部 Intent、Custom Tabs，或拦截并提示。不要对所有 URL 都盲目 `view.loadUrl(url)`，否则可能破坏 POST、重定向和系统处理；处理外部 Intent 前必须检查可解析目标并拒绝未知/危险 scheme。还应限制 JavaScript、文件访问和混合内容到业务所需的最小范围，不向不可信页面暴露 `addJavascriptInterface`，在 `onDestroy()` 销毁 WebView 并取消相关加载；加载失败、返回键历史和登录重定向也需单独设计与测试。
